...
The `home-routes` handler in the `cbook.routes.home` namespace
...
The `home-page` function will in turn call the `cbook.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/cbook/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `cbook.handler` namespace:
...
Request middleware functions are located under the `cbook.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `cbook.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `cbook.db.core` in some other namespace.
